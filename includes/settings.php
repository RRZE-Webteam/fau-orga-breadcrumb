...
function fau_orga_breadcrumb_field_callback(): void
{
    ...
    // Fakultät aus neuer zentraler Funktion ermitteln
    $faculty_slug = get_fau_faculty_by_theme();

    // Soll die "Keine"-Option angezeigt werden?
    $show_none = true;

    if (fau_is_elemental_theme()) {
        $type = fau_elemental_site_type();
        // Bei Fakultäts-, Lehrstuhl- oder Department-Websites → keine "Keine"-Option
        if (in_array($type, ['faculty', 'chair'], true) || ($type === 'other' && $faculty_slug !== '')) {
            $show_none = false;
        }
    } else {
        // Alte Themes: Typ 1 = Einrichtung einer Fakultät → keine "Keine"-Option
        $legacy_type = get_theme_mod('website_type');
        if ($legacy_type === 1) {
            $show_none = false;
        }
    }
...
function fau_orga_customizer_settings($wp_customize): void
{
    ...
    // Prüfen, ob das FAU.ORG-Plugin im FAU-Theme-Kontext läuft
    global $fau_orga_fautheme;
    if ($fau_orga_fautheme === false) {
        return;
    }

    // Aktuell gespeicherten Wert laden
    $options = get_option('fau_orga_breadcrumb_options');
    $orga = isset($options['site-orga']) ? esc_attr($options['site-orga']) : '0000000000';

    // Website-Typ für Optionsliste laden
    $website_type = get_theme_mod("website_type");

    // Optionsliste aufbauen
    $optionlist = '';
    ...
    // Standard-Sektion ist "Website-Informationen"
